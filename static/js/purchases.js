...
function InputBg() {
    ...
    $(function () {
        $(document).on('focus', '.qtypurchased', function () {
            $(this).css({'background-color': 'rgb(200,250,200)'});
        });
    });
    $(function () {
        $(document).on('blur', '.qtypurchased', function () {
            $(this).css({'background-color': 'rgb(255,255,255)'});
        });
    });
...
function Calculations(rowID) {
    ...
    qty_sold = $("#" + rowID + " .qtypurchased").val();
...
function SubCalculations(rowID) {
    ...
    qtypurchased = $("#" + rowID + " .qtypurchased").val();
    price = $("#" + rowID + " .price").val();
    net = parseInt(qtypurchased) * parseFloat(price).toFixed(3);
    d = net * (d_p / 100) || 0.000;
...
$(document).ready((function () {
    ...
    $(document).on('mousedown', '.delete-row', function () {
    ...
    $(document).on("click", "#addpurchase_button", function () {
        $(".product").not(':last').prop("disabled", false).trigger("chosen:updated");
        $('#addpurchase_form').submit();
    });
  ...
  // balance calculations
    ...
    $(document).on("keyup", ".qtypurchased", function () {
        var trid = $(this).parents("tr").attr("id");
        ...
        // } else
            ...
            sweetAlert({
                title: "Oops!",
                text: "Purchasing qty can't be 0 less than zero !",
                type: "error"
            });
            $(this).val(1).select();
    ...
    $(document).on("keyup", ".price", function () {
        ...
        qtypurchased = $("#" + trid + " .qtypurchased").val();
        ...
        $.ajax({
            ...
            success: function (data) {
                if (Obj.find_id(id) == undefined) {
                    ...
                    $("#" + trid + " .qtypurchased").val(1);
                    $("#" + trid + " .barcode").val(prePopulate[0].barcode);
                    $("#" + trid + " .price").val(prePopulate[0].purchase_price);
                    ...
                    qtypurchased = $("#" + Obj.find_id(id).row + " .qtypurchased").val();
                    $("#" + Obj.find_id(id).row + " .qtypurchased").val(parseInt(qtypurchased) + 1);
                    //check on qtysold
                    // var q = $("#" + Obj.find_id(id).row + " .qtypurchased").val();
    ...
    $(document).on("change", "#id_supplier", function () {
        ...
        $.ajax({
            url: '/purchases/sup_balance?id=' + id,
            type: 'GET',
            success: function (data) {
                prePopulate = $.parseJSON(data);
...
//
//
//         $(document).on('submit', '#add_sale', function () {
//             $.ajax({
//                 type: 'POST',
//                 url: "/cash_received/allocate/",
//                 data: $(this).serialize(),
//                 success: function (data) {
//                     swal({
...
//         });
    ...
    $(document).on("change", ".barcode", function () {
        ...
        $.ajax({
            url: '/purchases/pproduct?barcode=' + barcode,
            type: 'GET',
            success: function (data) {
                if (Obj.find_barcode(barcode) == undefined) {
                    ...
                    qtypurchased = $("#" + Obj.find_barcode(barcode).row + " .qtypurchased").val();
                    $("#" + Obj.find_barcode(barcode).row + " .qtypurchased").val(parseInt(qtypurchased) + 1);
                    // check on qtysold
                    // var q = $("#" + Obj.find_barcode(barcode).row + " .qtypurchased").val();
